...
class NotificationHelper {
  static late FlutterLocalNotificationsPlugin _fln;
  ...
  static Future<void> initialize(
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (message.data['type'] == 'message' &&
      ...
      } else if (_isDeliveryRequestNotification(message.data)) {
        debugPrint(
          '[MyardsOverlay] delivery-request-like payload detected in onMessage: ${message.data}',
        );
        ...
        if (type != 'assign' && !_isDeliveryRequestNotification(message.data)) {
          NotificationHelper.showNotification(
            message,
            flutterLocalNotificationsPlugin,
          );
  ...
  static Future<void> showTextNotification(
    ...
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails(
          'myards_delivery_requests',
          'Delivery Requests',
          channelDescription: 'New order requests for Myards Delivery Partners',
          playSound: true,
  ...
  static bool _isDeliveryRequestNotification(Map<String, dynamic> payload) {
    final String type = payload['type']?.toString().toLowerCase() ?? '';
    final String title = payload['title']?.toString().toLowerCase() ?? '';
    final String body = payload['body']?.toString().toLowerCase() ?? '';

    final bool typeMatches =
        (type.contains('order') || type.contains('request')) &&
        !type.contains('ride');
    final bool titleMatches =
        title.contains('delivery request') || title.contains('new order');
    final bool bodyMatches =
        body.contains('new order') || body.contains('delivery request');

    return typeMatches || titleMatches || bodyMatches;
  }

  static Future<void> handleNewDeliveryRequest(
    OrderModel order, {
    required bool fromNotification,
    RemoteMessage? sourceMessage,
  }) async {
    ...
    try {
      ...
      if (fromNotification) {
        debugPrint(
          '[MyardsOverlay] request queued from notification for orderId=${order.id}',
        );
      }
    } catch (e) {
      debugPrint(
        '[MyardsOverlay] queue/overlay failed for orderId=${order.id} error=$e',
      );
      if (sourceMessage != null) {
        NotificationHelper.showNotification(sourceMessage, _fln);
      }
    }
  }

  /// Handle new order/order_request notification - show overlay if app backgrounded
  static Future<void> _handleNewOrderNotification(RemoteMessage message) async {
    try {
      ...
      final latestList = Get.find<OrderController>().latestOrderList;
      if (latestList != null && latestList.isNotEmpty) {
        ...
        final latestOrder = latestList.first;

        await handleNewDeliveryRequest(
          latestOrder,
          fromNotification: true,
          sourceMessage: message,
        );
      } else {
        // No orders available, show regular notification
        NotificationHelper.showNotification(message, _fln);
      }
    } catch (e) {
      debugPrint('[MyardsOverlay] Error handling new order notification: $e');
      NotificationHelper.showNotification(message, _fln);
    }
  }
}
