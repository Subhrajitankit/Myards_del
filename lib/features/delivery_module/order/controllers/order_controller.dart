...
class OrderController extends GetxController implements GetxService {
  ...
  bool _hasSeededDeliverySnapshot = false;
  final Map<String, String> _lastKnownDeliveryStatuses = <String, String>{};
  ...
  Future<void> getLatestOrders() async {
    ...
    if (latestOrderList != null) {
      ...
      await MyardsOverlayHelper.processPendingOrdersSnapshot(
        _latestOrderList!,
        allowInitialSinglePopup: true,
      );
      await _processDeliveryRequestNotificationDiff(_latestOrderList!);
    }
    update();
  }

  /// Returns true if [createdAt] is within the last 10 minutes.
  bool _isRecentDeliveryOrder(String? createdAt, {int minutes = 10}) {
  ...
  Future<void> _processDeliveryRequestNotificationDiff(
    List<OrderModel> latestOrders,
  ) async {
    final Map<String, String> current = <String, String>{};
    for (final OrderModel order in latestOrders) {
      if (order.id == null) {
        continue;
      }
      final String orderId = order.id.toString();
      current[orderId] = order.orderStatus ?? 'pending';
    }

    if (!_hasSeededDeliverySnapshot) {
      // First run: load from SharedPreferences.
      final Map<String, String> saved = await MyardsLocalNotificationHelper.readSnapshot(
        MyardsLocalNotificationHelper.snapshotDeliveryRequestIds,
      );
      _lastKnownDeliveryStatuses.addAll(saved);
      _hasSeededDeliverySnapshot = true;
      debugPrint('[MyardsNotification] delivery request snapshot loaded from SharedPreferences: ${saved.length} orders');

      if (saved.isEmpty) {
        // First run: seed snapshot. Notify and trigger overlay for any recent request.
        for (final OrderModel order in latestOrders) {
          if (order.id == null) continue;
          final String orderId = order.id.toString();
          _lastKnownDeliveryStatuses[orderId] = order.orderStatus ?? 'pending';

          if (_isRecentDeliveryOrder(order.createdAt)) {
            await MyardsLocalNotificationHelper.showNotification(
              channelId: MyardsLocalNotificationHelper.channelDeliveryRequests,
              title: 'New Delivery Request',
              body:
                  'Pickup from ${order.storeName ?? 'Restaurant'} • ${order.detailsCount ?? 0} items • ${order.paymentMethod ?? 'payment'}',
              dedupeKey: 'delivery_request_${orderId}_new',
            ...
            await MyardsOverlayHelper.handleIncomingOrderRequest(order);
            debugPrint(
              '[MyardsNotification] delivery notification and overlay shown for recent '
              'seeded request $orderId',
            );
          }
        }
      } else {
        // Compare current status with saved snapshot for missed status updates/new orders
        for (final MapEntry<String, String> entry in current.entries) {
          final String orderId = entry.key;
          final String status = entry.value;
          final String? previousStatus = _lastKnownDeliveryStatuses[orderId];

          if (previousStatus == null) {
            final OrderModel order = latestOrders.firstWhere(
              (OrderModel e) => e.id.toString() == orderId,
            );
            await MyardsLocalNotificationHelper.showNotification(
              channelId: MyardsLocalNotificationHelper.channelDeliveryRequests,
              title: 'New Delivery Request',
            ...
            await MyardsLocalNotificationHelper.showNotification(
              channelId: MyardsLocalNotificationHelper.channelOrderUpdates,
              title: 'Order Update',
              body: 'Order #$orderId is $status.',
              dedupeKey: 'delivery_request_${orderId}_$status',
              payload: <String, dynamic>{
                'route_type': 'order_details',
                'order_id': orderId,
              },
              ttlMinutes: 20,
            );
      ...
      _lastKnownDeliveryStatuses
        ..clear()
        ..addAll(current);
      await MyardsLocalNotificationHelper.writeSnapshot(
        MyardsLocalNotificationHelper.snapshotDeliveryRequestIds,
        current,
      );
      return;
